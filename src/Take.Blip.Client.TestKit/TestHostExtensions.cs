...
namespace Take.Blip.Client.TestKit
{
    public static class TestHostExtensions
    {
        ...
        /// <returns></returns>
        public static Task<IServiceContainer> AddRegistrationAndStartAsync(
            this TestHost host,
            params object[] implementationOverrides
        )
        {
            return host.StartAsync(c =>
            {
                foreach (
                    var registration in implementationOverrides.SelectMany(implementation =>
                        implementation
                            .GetType()
                            .GetInterfaces()
                            .Where(t =>
                                !t.Namespace.StartsWith("System")
                                && !t.Namespace.StartsWith("Castle")
                                && !t.Namespace.StartsWith("NSubstitute")
                            )
        ...
        public static Task<Message> DeliverIncomingMessageAsync(
            this TestHost host,
            Node from,
            string plainContent
        )
        {
            return DeliverIncomingMessageAsync(
                host,
                from,
                (Document)new PlainText { Text = plainContent }
            );
        }

        public static async Task<Message> DeliverIncomingMessageAsync(
            this TestHost host,
            Node from,
            Document content
        )
        ...
        public static async Task WaitForConsumedAsync(this TestHost host, string messageId = null)
        {
            ...
            if (notification == null)
                throw new InvalidOperationException(
                    $"Expected Received notification, but could not get it within current timeout"
                );
            if (
                notification.Event != Event.Received
                && (messageId == null || notification.Id == messageId)
            )
            {
                throw new InvalidOperationException(
                    $"Expected Received notification, but got {notification.Event}"
                );
            }
            ...
            if (notification == null)
                throw new InvalidOperationException(
                    $"Expected Consumed notification, but could not get it within current timeout"
                );
            if (
                notification.Event != Event.Consumed
                && (messageId == null || notification.Id == messageId)
            )
            {
                var reason = "";
                if (notification.Event == Event.Failed)
                    reason = $" ({notification.Reason.Description})";
                throw new InvalidOperationException(
                    $"Expected Consumed notification, but got {notification.Event}{reason}"
                );
            }
            ;
        }

        public static async Task<IEnumerable<Message>> DeliverMessageAndConsumeResponseAsnyc(
            this TestHost host,
            Node from,
            string plainContent
        )
        ...
        public static async Task<Message> GetResponseIgnoreFireAndForgetAsync(
            this TestHost host,
            bool waitForConsumed = true
        )
        {
            if (waitForConsumed)
                await host.WaitForConsumedAsync();
            while (true)
            {
                ...
                if (!string.IsNullOrWhiteSpace(outgoing.Id))
                    return outgoing;
            }
        }

        public static async Task<T> GetContentResponseIgnoreFireAndForgetAsync<T>(
            this TestHost host,
            bool waitForConsumed = true
        )
            where T : Document
