...
namespace Take.Blip.Client.TestKit
{
    public class TestHost
    {
        ...
        public TestHost(
            Assembly assembly,
            TimeSpan? messageWaitTimeout = null,
            TimeSpan? notificationWaitTimeout = null,
        ...
        )
        {
            ...
            _typeResolver =
                typeResolver ?? new TypeResolver(new AssemblyProvider(typeof(BlipClient).Assembly));
        }

        public async Task<IServiceContainer> StartAsync(
        ...
        )
        {
            ...
            var application = Application.ParseFromJsonFile(
                Path.Combine(GetAssemblyRoot(), applicationFileName)
            );
            var typeResolver = new AggregateTypeResolver(
                new TypeResolver(new AssemblyProvider(_assembly)),
                _typeResolver
            );
            ...
            Identity = Identity.Parse(
                $"{application.Identifier}@{application.Domain ?? "msging.net"}"
            );
        ...
        private IServiceContainer BuildServiceContainer(
