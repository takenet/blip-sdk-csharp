...
namespace Take.Blip.Client.Session
{
    public class SessionManager : ISessionManager
    {
        ...
        )
        {
            ...
            var session = await GetOrCreateSessionAsync(node, cancellationToken)
        ...
        )
        {
            ...
            if (session?.Variables == null || !session.Variables.ContainsKey(key))
                return null;
            return session.Variables[key];
        }

        public async Task RemoveVariableAsync(
            Node node,
            string key,
            CancellationToken cancellationToken
        )
        ...
        public Task<NavigationSession> GetSessionAsync(
            Node node,
            CancellationToken cancellationToken
        ...
        private async Task<NavigationSession> GetOrCreateSessionAsync(
            Node node,
            CancellationToken cancellationToken
        ) =>
            await GetSessionAsync(node, cancellationToken)
            ?? new NavigationSession() { Creation = DateTimeOffset.UtcNow };
