...
namespace Take.Blip.Client.Extensions.Tunnel
{
    ...
    {
        public static bool TryGetTunnelFromEnvelope<T>(
            this T envelope,
            out Takenet.Iris.Messaging.Resources.Tunnel tunnelInformation
        )
            where T : Envelope
        {
            ...
            if (
                ...
                && envelope.Metadata != null
                && envelope.Metadata.TryGetValue(
                    TunnelExtension.TUNNEL_OWNER_METADATA_KEY,
                    out var owner
                )
                && envelope.Metadata.TryGetValue(
                    TunnelExtension.TUNNEL_ORIGINATOR_METADATA_KEY,
                    out var originator
                )
            )
            {
