...
namespace Take.Blip.Client.Extensions.Tunnel
{
    ...
    {
        public static readonly Node TunnelAddress = Node.Parse(
            $"postmaster@tunnel.{Constants.DEFAULT_DOMAIN}"
        );
        ...
        public const string TUNNEL_ORIGINATOR_METADATA_KEY =
            TUNNEL_METADATA_KEY_PREFIX + ".originator";
        ...
        public Task<Takenet.Iris.Messaging.Resources.Tunnel> GetTunnelAsync(
        ...
        public Task<Node> ForwardNotificationAsync(
            ...
            ForwardAsync(
                notification,
                destination,
                _sender.SendNotificationAsync,
                cancellationToken
            );

        private async Task<Node> ForwardAsync<TEnvelope>(
            TEnvelope envelope,
            Identity destination,
            Func<TEnvelope, CancellationToken, Task> senderFunc,
            CancellationToken cancellationToken
        )
            where TEnvelope : Envelope, new()
        {
            ...
            if (envelope.From == null)
                throw new ArgumentException(
                    "The envelope 'from' value must be provided",
                    nameof(envelope)
                );
            if (destination == null)
