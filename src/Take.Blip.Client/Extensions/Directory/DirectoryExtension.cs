...
namespace Take.Blip.Client.Extensions.Directory
{
    public class DirectoryExtension : ExtensionBase, IDirectoryExtension
    {
        ...
        public DirectoryExtension(ISender sender)
            : base(sender) { }

        public Task<Account> GetDirectoryAccountAsync(
            Identity identity,
        ...
        )
        {
            ...
            if (string.IsNullOrWhiteSpace(identity.Domain))
                throw new ArgumentException("Invalid identity domain", nameof(identity));
