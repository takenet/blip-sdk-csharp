...
namespace Take.Blip.Client.Extensions.Context
{
    public class ContextExtension : ExtensionBase, IContextExtension
    {
        ...
        public static readonly Identity GlobalIdentity = new Identity(
            "*",
            Constants.DEFAULT_DOMAIN
        );

        public ContextExtension(ISender sender)
            : base(sender) { }

        public Task<T> GetVariableAsync<T>(
            Identity identity,
            string variableName,
        ...
        private static string AppendExpiration<T>(string uriPath, TimeSpan expiration)
            where T : Document
        {
