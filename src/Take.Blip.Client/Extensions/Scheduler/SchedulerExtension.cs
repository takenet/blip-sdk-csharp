...
namespace Take.Blip.Client.Extensions.Scheduler
{
    public class SchedulerExtension : ExtensionBase, ISchedulerExtension
    {
        ...
        private static readonly Node SchedulerAddress = Node.Parse(
            $"postmaster@scheduler.{Constants.DEFAULT_DOMAIN}"
        );
        ...
        )
        {
            ...
            return ProcessCommandAsync(
                CreateSetCommandRequest(
                    new Schedule { Message = message, When = when },
                    SCHEDULE_URI,
                    SchedulerAddress,
                    from: from
                ),
        ...
        public Task<Schedule> GetScheduledMessageAsync(
            string messageId,
            Node from = null,
        ...
        public Task CancelScheduledMessageAsync(
            string messageId,
            Node from = null,
            CancellationToken cancellationToken = new CancellationToken()
        )
