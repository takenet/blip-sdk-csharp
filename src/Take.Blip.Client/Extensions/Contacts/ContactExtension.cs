...
namespace Take.Blip.Client.Extensions.Contacts
{
    ...
    {
        private static readonly Node CrmAddress = Node.Parse(
            $"postmaster@crm.{Constants.DEFAULT_DOMAIN}"
        );
        ...
        public Task<Contact> GetAsync(Identity identity, CancellationToken cancellationToken)
        {
            ...
            var requestCommand = CreateGetCommandRequest(
                Smart.Format(
                    CONTACT,
                    new { contactIdentity = Uri.EscapeDataString(identity.ToString()) }
                ),
                CrmAddress
            );
        ...
        )
        {
            ...
            var requestCommand = CreateSetCommandRequest(contact, CONTACTS, CrmAddress);
        ...
        )
        {
            ...
            var requestCommand = CreateMergeCommandRequest(contact, CONTACTS, CrmAddress);
