        ...
        public IContext CreateContext(Identity user, Identity application, LazyInput input, Flow flow)
        {
            switch (_configuration.ContextType)
            {
                case nameof(StorageContext):
                    return new StorageContext(user, application, input, flow, _variableProviders.Value, _ownerCallerNameDocumentMap.Value);

                default:
                    return new ExtensionContext(user, application, input, flow, _variableProviders.Value, _contextExtension.Value);
            }
        }
    }
