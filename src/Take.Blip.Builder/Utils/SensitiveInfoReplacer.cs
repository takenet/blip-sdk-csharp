...
namespace Take.Blip.Builder.Utils
{
    ...
    public class SensitiveInfoReplacer : ISensitiveInfoReplacer
    {
        ...
        public string ReplaceCredentials(string value)
        {
            ...
            var httpSettings = JsonConvert.DeserializeObject<ProcessHttpSettings>(
                value,
                OrderedJsonSerializerSettingsContainer.Settings
            );
            ...
            {
                var maskedBody = Regex.Replace(
                    httpSettings.Body,
                    PATTERN,
                    DEFAULT_VALUE_FOR_SUPRESSED_STRING
                );
            ...
            {
                httpSettings.Uri = new Uri(
                    DEFAULT_VALUE_FOR_SUPRESSED_STRING,
                    UriKind.RelativeOrAbsolute
                );
            }

            var adjustedValue = JsonConvert.SerializeObject(
                httpSettings,
                OrderedJsonSerializerSettingsContainer.Settings
            );
