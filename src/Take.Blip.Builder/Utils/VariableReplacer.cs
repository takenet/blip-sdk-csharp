...
namespace Take.Blip.Builder.Utils
{
    ...
    {
        private static readonly Regex TextVariablesRegex = new Regex(
            @"{{([a-zA-Z0-9\.@_-]+)}}",
        ...
        )
        {
            ...
            foreach (Match match in TextVariablesRegex.Matches(value))
            {
                ...
                if (variableValues.ContainsKey(variableName))
                    continue;
                var variableValue = await context.GetVariableAsync(
                    variableName,
                    cancellationToken,
                    stateActionType
                );
            ...
            {
                value = TextVariablesRegex.Replace(
                    value,
                    match => variableValues[match.Groups[1].Value]
                );
            }
