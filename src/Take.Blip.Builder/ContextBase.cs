...
namespace Take.Blip.Builder
{
    public abstract class ContextBase : IContext
    {
        ...
        private struct VariableName
        {
            ...
            private static readonly Regex VariableNameRegex = new Regex("^(?<sourceOrName>[\\w\\d]+)(\\.(?<name>[\\w\\d\\.]+))?(@(?<property>([\\w\\d\\.](\\[(?<index>\\d+|\\$n)\\])?)+))?$", RegexOptions.Compiled | RegexOptions.IgnoreCase, TimeSpan.FromMinutes(2));
