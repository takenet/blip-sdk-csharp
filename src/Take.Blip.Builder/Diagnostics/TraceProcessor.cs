...
namespace Take.Blip.Builder.Diagnostics
{
    public class TraceProcessor : ITraceProcessor
    {
        ...
        public TraceProcessor(IHttpClient httpClient, ISender sender)
        {
        ...
        public Task ProcessTraceAsync(TraceEvent traceEvent, CancellationToken cancellationToken)
        {
            ...
            switch (traceEvent.Settings.TargetType)
            {
                case TraceTargetType.Http:
                    return ProcessHttpTraceAsync(
                        new Uri(traceEvent.Settings.Target),
                        traceEvent.Trace,
                        cancellationToken
                    );

                case TraceTargetType.Lime:
                    return ProcessLimeTraceAsync(
                        traceEvent.Settings.Target,
                        traceEvent.Trace,
                        cancellationToken
                    );
                ...
                default:
                    throw new NotSupportedException(
                        $"Unsupported trace target '{traceEvent.Settings.TargetType}'"
                    );
            }
        }

        private async Task ProcessHttpTraceAsync(
            Uri uri,
            InputTrace trace,
        ...
        private async Task ProcessLimeTraceAsync(
            Node node,
            InputTrace trace,
            CancellationToken cancellationToken
        )
        {
            await _sender.SendMessageAsync(
                new Message { To = node, Content = trace },
                cancellationToken
            );
        }
    }
