    ...
    public class ManageListAction : ActionBase<ManageListSettings>
    {
        ...
        )
        {
            ...
            while (tryCount-- > 0)
            {
                try
                {
                    await _broadcastExtension.AddRecipientAsync(
                        listName,
                ...
                catch (LimeException ex) when (ex.Reason.Code == ReasonCodes.APPLICATION_ERROR)
                {
                    ...
                    await _broadcastExtension.CreateDistributionListAsync(
                        listName,
                        cancellationToken
                    );
                }
        ...
        )
        {
            try
            {
                await _broadcastExtension.DeleteRecipientAsync(
                    listName,
                    context.UserIdentity,
                    cancellationToken
                );
                ...
                when (ex.Reason.Code == ReasonCodes.APPLICATION_ERROR
