...
namespace Take.Blip.Builder.Actions.ExecuteTemplate
{
    public class ExecuteTemplateAction : ActionBase<ExecuteTemplateSettings>
    {
        ...
        {
            nameof(ExecuteTemplateSettings.OutputVariable).ToCamelCase(),
        };
        ...
        public override async Task ExecuteAsync(
            IContext context,
            ExecuteTemplateSettings settings,
        ...
        )
        {
            ...
            try
            {
                ...
                try
                {
                    var arguments = await GetScriptArgumentsAsync(
                        context,
                        settings,
                        cancellationToken
                    );
                ...
                catch (Exception ex)
                {
                    if (ex is HandlebarsParserException)
                    {
                        _logger.Warning(
                            ex,
                            "Unexpected error while trying to parse Handlebars template"
                        );
                        throw;
                    }
        ...
        private Dictionary<string, object> CopyArgumentsToDictionary(JObject arguments)
        {
            ...
            foreach (var property in arguments.Properties())
            {
                try
                {
                    var deserializedJson = JsonConvert.DeserializeObject(
