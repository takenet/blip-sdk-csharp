...
namespace Take.Blip.Builder.Actions.ProcessHttp
{
    public sealed class ProcessHttpAction : ActionBase<ProcessHttpSettings>, IDisposable
    {
        ...
        public ProcessHttpAction(
            IHttpClient httpClient,
            ILogger logger,
            IConfiguration configuration,
            ISensitiveInfoReplacer sensitiveInfoReplacer,
            ...
            try
            {
                ...
                using (
                    var httpRequestMessage = new HttpRequestMessage(
                        new HttpMethod(settings.Method),
                        settings.Uri
                    )
                )
                {
                    if (settings.Headers != null)
                    {
                        ...
                        {
                            var secret = await _variableReplacer.ReplaceAsync(
                                header.Value,
                                context,
                                cancellationToken,
                                ACTION_PROCESS_HTTP
                    ...
                    if (!string.IsNullOrWhiteSpace(settings.Body))
                    {
                        ...
                        requestBody = await _variableReplacer.ReplaceAsync(
                            settings.Body,
                            context,
                            cancellationToken,
                            ACTION_PROCESS_HTTP
                    ...
                    {
                        AddHeadersToCommandRequest(
                            httpRequestMessage,
                            settings.currentStateId,
                ...
                if (
                    !string.IsNullOrWhiteSpace(settings.ResponseBodyVariable)
                ...
                this.LogExecution(
                    ...
                    new JObject
                    {
                        ...
                        ["headers"] =
                            settings.Headers != null ? JObject.FromObject(settings.Headers) : null,
                    }
                );
            }
        ...
        /// <param name="ex"></param>
        private void BuildHttpErrorMessage(
            ProcessHttpSettings settings,
        ...
        )
        {
            ...
            if (ex.InnerException != null)
            {
                ...
                while (innerEx != null)
                {
                    ...
                    if (
                        exMessage.Contains("certificate")
                    ...
                    )
                    {
                        ...
                        )
                        {
                            ...
                            errorDetails.AppendLine(
                                "Certificate trust chain validation failed (Untrusted Root)"
                            );
                        }
                    }
            ...
            _logger.Error(
                ex,
                "HTTP request failed for URL: {Uri}. Details: {Details}",
                settings.Uri,
        ...
        /// <param name="httpRequestMessage"></param>
        /// <param name="context"></param>
        private void AddBotIdentityToHeaders(
        ...
        private void AddHeadersToCommandRequest(
            HttpRequestMessage httpRequestMessage,
