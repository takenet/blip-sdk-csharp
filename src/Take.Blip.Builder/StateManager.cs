...
namespace Take.Blip.Builder
{
    public class StateManager : IStateManager
    {
        ...
        public Task<string> GetPreviousStateIdAsync(
            IContext context,
            CancellationToken cancellationToken
        )
        {
            return context.GetContextVariableAsync(
                GetPreviousStateKey(context.Flow.Id),
                cancellationToken
            );
        }
        ...
        )
        {
            ...
            return context.GetContextVariableAsync(
                GetStateKey(context.Flow.Parent.Id),
                cancellationToken
            );
        }

        public Task SetStateIdAsync(
            IContext context,
            string stateId,
            CancellationToken cancellationToken
        )
        ...
        public Task SetPreviousStateIdAsync(
            IContext context,
        ...
        {
            return context.DeleteVariableAsync(
                GetStateKey(context.Flow.Parent?.Id),
                cancellationToken
            );
        }

        public Task DeleteCurrentFlowSessionAsync(
            IContext context,
            CancellationToken cancellationToken
        )
        {
            return context.DeleteVariableAsync(
                GetCurrentFlowSessionKey(context.Flow.Parent?.Id),
                cancellationToken
            );
        }

        public Task DeleteMasterStateAsync(IContext context, CancellationToken cancellationToken) =>
            context.DeleteVariableAsync(MASTER_STATE, cancellationToken);
        ...
        private static string GetCurrentFlowSessionKey(string flowId) =>
            $"{CURRENT_FLOW_SESSION_KEY}@{flowId}";

        private static string GetPreviousStateKey(string flowId) =>
            $"{PREVIOUS_STATE_PREFIX}-{STATE_ID_KEY}@{flowId}";
    }
}
