...
namespace Take.Blip.Builder.UnitTests
{
    public class InputExpirationHandlerTests
    {
        ...
        public InputExpirationHandlerTests()
        {
            ...
            InputHandler = new InputExpirationHandler(
                Scheduler,
                Logger,
                _inputExpirationCount,
                _configuration
            );
        }
        ...
        public async Task ValidateMessageDontChangeTraceMetadata()
        {
            ...
            var messageContent = new InputExpiration()
            {
            ...
            Assert.True(
                returnMessage.Metadata.ContainsKey(TraceSettings.BUILDER_TRACE_TARGET_TYPE)
            );
        ...
        public async Task WhenCreateInputExpirationMessageWithTraceMetadataThenTraceMetadataMustbePresent()
        {
            ...
            {
                Id = "ping",
                Input = new Builder.Models.Input() { Expiration = TimeSpan.FromMinutes(1) },
            };
            var flow = new Flow();

            // Act
            await InputHandler.OnFlowProcessedAsync(
                state,
                flow,
                Message,
                null,
        ...
        public async Task WhenUserSendsMessage_WithoutInputExpiration()
        {
            ...
            // Act
            await InputHandler.OnFlowPreProcessingAsync(
                state,
                Message,
                null,
                default(CancellationToken)
