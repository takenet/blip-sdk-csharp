    ...
    public class SubflowTests : FlowManagerTestsBase, IDisposable
    {
        ...
        public async Task FlowWithSubflowRedirectAndStateWithInputUserShouldSuccess()
        {
            ...
            var input2 = new Message()
            {
                From = UserIdentity.ToNode(),
                To = ApplicationIdentity.ToNode(),
                Content = new PlainText() { Text = "Ping Subflow!" },
            };
            ...
            ContextProvider
                ...
                .CreateContext(
                    UserIdentity,
                    ApplicationIdentity,
                    Arg.Is<LazyInput>(i => i.Content == input1.Content),
                    flow
                );
            ContextProvider
                .Received(1)
                .CreateContext(
                    UserIdentity,
                    ApplicationIdentity,
                    Arg.Is<LazyInput>(i => i.Content == input2.Content),
                    subflow
                );
            ...
            StateManager
                .Received(1)
                .SetStateIdAsync(context1, "ping", Arg.Any<CancellationToken>());
            StateManager
                .Received(1)
                .SetStateIdAsync(context1, "subflow:subflowTest", Arg.Any<CancellationToken>());
            StateManager
                .Received(1)
                .SetStateIdAsync(context1, "root", Arg.Any<CancellationToken>());
            StateManager
                .Received(1)
                .SetStateIdAsync(context1, "pingSubflow", Arg.Any<CancellationToken>());
            StateManager.Received(1).SetStateIdAsync(context2, "end", Arg.Any<CancellationToken>());
            StateManager
                .Received(1)
                .SetStateIdAsync(context2, "ping2", Arg.Any<CancellationToken>());

            context1
                .Received(1)
                .SetVariableAsync(
                    "testSetVariableInput",
                    "testSetVariableInputValue",
                    Arg.Any<CancellationToken>(),
                    Arg.Any<TimeSpan>()
                );
            context2
                .Received(1)
                .SetVariableAsync(
                    "testSetVariableOutput",
                    "testSetVariableOutputValue",
                    Arg.Any<CancellationToken>(),
                    Arg.Any<TimeSpan>()
                );
            context2
                .Received(1)
                .SetVariableAsync(
                    "testSetVariableAfterStateChanged",
                    "testSetVariableAfterStateChangedValue",
                    Arg.Any<CancellationToken>(),
                    Arg.Any<TimeSpan>()
                );
        ...
        public async Task FlowWithSubflowRedirectAndStateWithoutInputUserShouldSuccess()
        {
            ...
            StateManager
                .Received(1)
                .SetStateIdAsync(Context, "subflow:subflowTest", Arg.Any<CancellationToken>());
            StateManager.Received(1).SetStateIdAsync(Context, "root", Arg.Any<CancellationToken>());
            StateManager
                .Received(1)
                .SetStateIdAsync(Context, "pingSubflow", Arg.Any<CancellationToken>());
        ...
        public async Task SubflowWithOldVersionShouldFail()
        {
            ...
            // Act
            var exception = await Assert.ThrowsAsync<BuilderException>(async () =>
                await target.ProcessInputAsync(Message, flow, CancellationToken)
            );
            Assert.IsType<ValidationException>(exception.InnerException);

            await Assert.ThrowsAsync<ValidationException>(async () =>
                await target.ProcessInputAsync(Message, subflow, CancellationToken)
            );
        }
        ...
        public async Task RedirectToSubflowWithNullParentShouldFail()
        {
            ...
            Assert.Equal(
                $"Value cannot be null. (Parameter 'Error on return to parent flow of '{subflow.Id}'')",
                exception.InnerException.Message
            );
        }

        private static Flow CreateSubflowObject(
            string messageType,
            string messageContentSubflow,
            bool inputUserBeforeEnd
        )
        {
            return new Flow()
            {
                ...
                States = new[]
                {
                    new State
                    {
                        ...
                        Outputs = new[] { new Output { StateId = "pingSubflow" } },
                    },
                    new State
                    {
                        Id = "pingSubflow",
                        ...
                        Outputs = new[] { new Output { StateId = "end" } },
                    },
                    new State { Id = "end", End = true },
                },
            };
        }

        private static Flow CreateFlowObject(
            string subflowShortName,
            string messageType,
            ...
            return new Flow()
            {
                ...
                States = new[]
                {
                    ...
                    new State
                    {
                        ...
                        Outputs = new[] { new Output { StateId = "subflow:subflowTest" } },
                    },
                    new State
                    {
