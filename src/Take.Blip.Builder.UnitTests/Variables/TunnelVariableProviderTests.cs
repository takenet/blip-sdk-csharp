...
namespace Take.Blip.Builder.UnitTests.Variables
{
    public class TunnelVariableProviderTests : ContextTestsBase
    {
        ...
        public async Task GetFromMessageShouldSucceed()
        {
            ...
            {
                {
                    Take.Blip.Client.Extensions.Tunnel.TunnelExtension.TUNNEL_OWNER_METADATA_KEY,
                    Owner
                },
                {
                    Take.Blip
                        ...
                        .Tunnel
                        .TunnelExtension
                        .TUNNEL_ORIGINATOR_METADATA_KEY,
                    Originator
                },
            };
            ...
            // Act
            var actualOriginator = await target.GetVariableAsync(
                "originator",
                Context,
                CancellationToken
            );
            var actualOwner = await target.GetVariableAsync("owner", Context, CancellationToken);
            var actualDestination = await target.GetVariableAsync(
                "destination",
                Context,
                CancellationToken
            );
            var actualIdentity = await target.GetVariableAsync(
                "identity",
                Context,
                CancellationToken
            );
