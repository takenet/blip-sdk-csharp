...
namespace Programmatically
{
    class Program
    {
        ...
        {
            var client = new BlipClientBuilder().UsingAccessKey("", "").Build();
            ...
            listener.AddNotificationReceiver(
                new LambdaNotificationReceiver(
                    ...
                    {
                        Console.WriteLine(
                            "Notification '{0}' received from '{1}': {2}",
                            notification.Id,
                            notification.From,
                            notification.Event
                        );
            ...
            listener.AddCommandReceiver(
                new LambdaCommandReceiver(
                    (command, cancellationToken) =>
                    {
                        Console.WriteLine(
                            "Command '{0}' received from '{1}': {2} {3}",
                            command.Id,
                            command.From,
                            command.Method,
                            command.Uri
