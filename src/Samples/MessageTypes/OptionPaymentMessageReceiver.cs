    ...
    public class OptionPaymentMessageReceiver : IMessageReceiver
    {
        ...
        public async Task ReceiveAsync(Message message, CancellationToken cancellationToken)
        {
            ...
            if (message.Content.ToString().Equals("pm1"))
            {
                ...
                var toPagseguro =
                    $"{Uri.EscapeDataString(message.From.ToIdentity().ToString())}@pagseguro.gw.msging.net";
        ...
        public async Task<InvoiceStatus> getPaymentReceipt(
            Message message,
            ...
            switch (invoiceStatus?.Status)
            {
                case InvoiceStatusStatus.Cancelled:
                    await _sender.SendMessageAsync(
                        "Tudo bem, n�o precisa pagar nada.",
                        message.From,
                        cancellationToken
                    );
                    break;
                case InvoiceStatusStatus.Completed:
                    await _sender.SendMessageAsync(
                        "Obrigado pelo seu pagamento, mas como isso � apenas um teste, voc� pode pedir o ressarcimento do valor pago ao PagSeguro. Em todo caso, segue o seu recibo:",
                        message.From,
                        cancellationToken
                    );
                ...
                case InvoiceStatusStatus.Refunded:
                    await _sender.SendMessageAsync(
                        "Pronto. O valor que voc� me pagou j� foi ressarcido pelo PagSeguro!",
                        message.From,
                        cancellationToken
                    );
                    break;
