namespace bot_flash_cards_blip_sdk_csharp
{
    ...
    public class StateMachine
    {
        ...
        public async Task<string> VerifyStateAsync(
            Message message,
        ...
        {
            Game game = _games.ContainsKey(message.From.ToIdentity().ToString())
                ? game = _games[message.From.ToIdentity().ToString()]
                : new Game();
            ...
            switch (await VerifyStateAsync(message, cancellationToken))
            {
                ...
                case "state-two":
                    ...
                    Thread.Sleep(1000);
                    await _sender.SendMessageAsync(
                        $"How many questions do you want to your game? The maximum is {game.People.Count}.",
                        message.From,
                        cancellationToken
                    );
                ...
                case "state-three":
                    ...
                    if (
                        Int32.TryParse(message.Content.ToString(), out questions)
                        && questions <= game.People.Count
                        && questions > 0
                    )
                    {
                ...
                case "state-four":
                    ...
                    else
                    {
                        ...
                        Thread.Sleep(1000);
                        await _sender.SendMessageAsync(
                            $"Yay! {game.Player}, your result is: {game.ProccesResult()}.",
                            message.From,
                            cancellationToken
                        );
                        Thread.Sleep(1000);
