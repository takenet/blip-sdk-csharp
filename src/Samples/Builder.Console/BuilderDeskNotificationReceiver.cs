...
namespace Builder.Console
{
    public class BuilderDeskNotificationReceiver : INotificationReceiver
    {
        ...
        )
        {
            ...
            if (
                notification.Event == Event.Accepted
            ...
            if (
                notification.From.Domain.Equals(
            ...
            {
                deskNotification.To = (
                    ticket.RoutingCustomerIdentity ?? ticket.CustomerIdentity
                ).ToNode();
            ...
            {
                deskNotification.To = Identity
                    .Parse($"{ticket.Id}@{HelpDeskExtension.DEFAULT_DESK_DOMAIN}")
                    .ToNode();
                deskNotification.Metadata = new Dictionary<string, string>
                {
        ...
        )
        {
            ...
            if (
                ...
                && Identity.TryParse(
                    Uri.UnescapeDataString(notification.From.Name),
                    out var encodedUserIdentity
                )
            )
            {
            ...
            if (stateId == null || !stateId.StartsWith("desk:"))
                return null;
        ...
        private Task<Ticket> GetCustomerActiveTicketAsync(
